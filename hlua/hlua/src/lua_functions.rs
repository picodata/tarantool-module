    ...
    where L: AsMutLua<'lua>
{
    ...
    /// but there is no check for the correct number of values. If you want a
    /// to be sure the number of values is correct, use [`call_multi`] instead.
    /// * If the expected number of values is less than the actual, only the
    ///   first few values will be returned.
    /// * If the expected number of values is greater than the actual, this is
    ///   **undefined behavior**.
    ///
    ...
    /// ```
    ///
    /// [`call_multi`]: LuaFunction::call_multi
    #[inline]
        ...
        V: LuaRead<PushGuard<&'a mut L>>
    {
        ...
        let (pcall_return_value, n_results_actual) = unsafe {
        ...
        if let Some(n_results_expected) = n_results {
            if n_results_expected != n_results_actual as u32 {
                return Err(
                    LuaError::wrong_type::<V, _>(pushed_value, n_results_actual)
                        .into()
                )
            }
        }

        LuaRead::lua_read_at_position(pushed_value, -n_results_actual)
            .map_err(|lua| LuaError::wrong_type::<V, _>(lua, n_results_actual).into())
    }

    /// Calls the function with parameters expecting multiple return values.
    ///
    /// Will check if the correct number of values was returned.
    ///
    /// You can either pass a single value by passing a single value, or
    /// multiple parameters by passing a tuple.
    /// If you pass a tuple, the first element of the tuple will be the first
    /// argument, the second element of the tuple the second argument, and so
    /// on.
    ///
    /// Returns an error if there is an error while executing the Lua code (eg.
    /// a function call returns an error), if the requested return type doesn't
    /// match the actual return type, or if we failed to push an argument.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// let result: (i32, i32) = foo.call_multi((18, 4)).unwrap();
    /// assert_eq!(result, (4, 2));
    /// ```
    #[inline]
    pub fn call_multi<'a, V, A, E>(&'a mut self, args: A) -> Result<V, LuaFunctionCallError<E>>
    where
        A: for<'r> Push<&'r mut LuaFunction<L>, Err = E>,
        V: LuaRead<PushGuard<&'a mut L>>,
        V: ReturnCount,
    {
        self.call_impl(args, Some(V::RETURN_COUNT))
    }
