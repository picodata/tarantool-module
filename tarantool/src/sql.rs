    ...
    IN: Serialize,
{
    ...
    let mut _param_data = Vec::new();
    let mut param_ptr = std::ptr::null::<u8>();
    if std::mem::size_of::<IN>() != 0 {
        _param_data = rmp_serde::to_vec(bind_params)?;
        debug_assert!(crate::msgpack::skip_value(&mut std::io::Cursor::new(&_param_data)).is_ok());
        param_ptr = _param_data.as_ptr() as *const u8;
    }
...
pub fn prepare(query: String) -> crate::Result<Statement> {
    ...
    if unsafe {
        ffi::sql::sql_prepare_ext(
            query.as_ptr(),
            query.len() as u32,
            &stmt_id as *const u32 as *mut u32,
        )
    } < 0
    {
        return Err(TarantoolError::last().into());
...
impl Statement {
        ...
        IN: Serialize,
    {
        ...
        if std::mem::size_of::<IN>() != 0 {
            _param_data = rmp_serde::to_vec(bind_params)?;
            debug_assert!(
                crate::msgpack::skip_value(&mut std::io::Cursor::new(&_param_data)).is_ok()
            );
            param_ptr = _param_data.as_ptr() as *const u8;
        }
        let execute_result = unsafe {
