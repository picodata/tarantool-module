...
use crate::util::IntoTupleOfClones;
...
impl<T> Drop for LuaJoinHandle<T> {
    fn drop(&mut self) {
        if let Some(fiber_ref) = self.fiber_ref {
            // In case join wasn't called on this fiber before the join handle
            // was dropped, we need to make the fiber not joinable so that it
            // can be recycled when it eventually finishes execution
            unsafe {
                impl_details::lua_fiber_set_joinable_and_unref(fiber_ref)
            }
            .map_err(|e| panic!("Unrecoverable lua failure: {}", e))
            .unwrap()
        }
...
impl<T> Drop for JoinHandle<T> {
    fn drop(&mut self) {
        if let Some(inner) = self.inner {
            ...
            // was dropped, we need to make the fiber not joinable so that it
            // can be recycled when it eventually finishes execution
            unsafe { ffi::fiber_set_joinable(inner.as_ptr(), false) }
        }
    }
}
    ...
    pub fn send(&self, t: T, timeout: Option<Duration>) -> StdResult<(), SendError<T>> {
        ...
        unsafe {
            ...
            if ret_code < 0 {
                ...
                // XXX: this is the cheapes way to check if the timeout
...
impl<T> Drop for Sender<T> {
    fn drop(&mut self) {
        let old = self.chan.tx_count.get();
        self.chan.tx_count.set(old - 1);
    }
}
...
impl<T> Drop for Receiver<T> {
    fn drop(&mut self) {
        let old = self.chan.rx_count.get();
        self.chan.rx_count.set(old - 1);
    }
}
...
where
    Sender<T>: IntoTupleOfClones<S>,
    Receiver<T>: IntoTupleOfClones<R>,
{
    let chan = Rc::new(Channel::new(size));
    (Sender::new(chan.clone()).clones(), Receiver::new(chan).clones())
}
