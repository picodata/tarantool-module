...
use crate::lua::value::{FromLuaMulti, MultiValue, ToLuaMulti};
...
impl Function {
    ...
    pub fn call<A: ToLuaMulti, R: FromLuaMulti>(&self, args: A) -> Result<R> {
        ...
        let args = args.to_lua_multi(ctx)?;
        let nargs = args.len() as c_int;

        let results = unsafe {
            let _sg = StackGuard::new(ctx.state);
            check_stack(ctx.state, nargs + 3)?;

            ffi::lua_pushcfunction(ctx.state, error_traceback);
            let stack_start = ffi::lua_gettop(ctx.state);
            ctx.push_ref(&self.0);
            for arg in args {
                ctx.push_value(arg)?;
            }
            let ret = ffi::lua_pcall(ctx.state, nargs, ffi::LUA_MULTRET, stack_start);
            if ret != ffi::LUA_OK {
                return Err(pop_error(ctx.state, ret));
            }
            let nresults = ffi::lua_gettop(ctx.state) - stack_start;
            let mut results = MultiValue::new();
            assert_stack(ctx.state, 2);
            for _ in 0..nresults {
                results.push_front(ctx.pop_value());
            }
            ffi::lua_pop(ctx.state, 1);
            results
        };
        R::from_lua_multi(results, &ctx)
    }
    ...
    pub fn bind<A: ToLuaMulti>(&self, args: A) -> Result<Function> {
        ...
        unsafe {
            let _sg = StackGuard::new(ctx.state);
            check_stack(ctx.state, nargs + 5)?;
            ctx.push_ref(&self.0);
            ffi::lua_pushinteger(ctx.state, nargs as ffi::lua_Integer);
            for arg in args {
                ctx.push_value(arg)?;
            }

            protect_lua_closure(ctx.state, nargs + 2, 1, |state| {
                ffi::lua_pushcclosure(state, bind_call_impl, nargs + 2);
            })?;

            Ok(Function(ctx.pop_ref()))
        }
    }
}
