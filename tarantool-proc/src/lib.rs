...
use syn::{
    ...
    Lit, Meta, MetaNameValue, NestedMeta, Pat, PatType, Signature,
};
use quote::quote;
...
pub fn stored_proc(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let Context { tarantool, debug_tuple, .. } = Context::from_args(args);
    ...
    let input_idents = inputs.iter()
        ...
        .map(|p| match &**p {
            ...
            _ => {
                quote! {
                    "Unsupported function argument: only simple identifier argument patterns are supported"
                }
            }
        })
        .collect::<Vec<_>>();

    let input_args = if input_idents.len() == 1 {
        let single_input_ident = &input_idents[0];
        quote! { #single_input_ident }
    } else {
        quote! { ( #(#input_idents),* ) }
    };
