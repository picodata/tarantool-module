...
func HasInstanceUUID(o metav1.ObjectMetaAccessor) bool {
	...
	if _, ok := annotations["tarantool.io/instance_uuid"]; ok {
		return true
	}
...
func SetInstanceUUID(o *corev1.Pod) *corev1.Pod {
	...
	oldAnnotations["tarantool.io/instance_uuid"] = instanceUUID.String()
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	err = c.Watch(&source.Kind{Type: &corev1.Pod{}}, &handler.EnqueueRequestForObject{}, &TarantoolPodPredicate{})
	if err != nil {
		return err
	}
...
func (r *ReconcileTarantoolCluster) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if instance.GetObjectMeta().GetDeletionTimestamp().IsZero() {
		...
		req := fmt.Sprintf("mutation {join_instance: join_server(uri: \\\"%s\\\",instance_uuid: \\\"%s\\\",replicaset_uuid: \\\"%s\\\",roles: [\\\"router\\\"],timeout: 10)}", advURI, instanceUUID, replicasetUUID)
