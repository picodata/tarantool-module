...
fn collect_tests(bench: bool) -> Vec<TestDescAndFn> {
    if bench {
        // TODO: remove bench or move to another fn?
        vec![TestDescAndFn {
            desc: TestDesc {
...
fn run_test(cfg: TestConfig) -> Result<bool, io::Error> {
    unsafe { LISTEN = cfg.listen };
    let mut tests = collect_tests(cfg.bench);
    if cfg.print {
        println!(
            "TEST_NAMES_BEGIN{}TEST_NAMES_END",
            serde_json::to_string(&TestNames {
                test_names: tests
                    .iter()
                    .map(|test| test.desc.name.to_string())
                    .collect()
            })
            .unwrap()
        );
        ...
        tests.retain(|test| test.desc.name.to_string() == cfg.test_name);
        run_tests_console(
            &TestOpts {
                list: false,
                filter: None,
...
pub unsafe extern "C" fn start(l: *mut ffi_lua::lua_State) -> c_int {
    ...
    let is_success = match run_test(cfg) {
        Ok(success) => success,
        Err(e) => {
