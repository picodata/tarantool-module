...
pub fn tuple_field_count() {
    ...
    // empty tuple
    let tuple = Tuple::encode_rmp(&()).unwrap();
...
pub fn tuple_get_field_path() {
    ...
    let tuple = idx_1
        .get(&Tuple::encode_rmp(&("key_16",)).unwrap())
        .unwrap()
        .unwrap();
...
pub fn tuple_compare_with_key() {
    ...
    assert_eq!(
        key_def.compare_with_key(&tuple, &Tuple::encode_rmp(&[0x1d]).unwrap()),
        Ordering::Equal
    );
